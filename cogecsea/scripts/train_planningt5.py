...
from cogecsea.models.T5seq2seq import CustomDataset_planning, T5Planning
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init_model', type=str, dest='init_model', default=None,
    ...
    if(args.check==False):
        ...
        if (args.val_file is None):
            ...
            val_dataset=pd.read_csv(args.val_file,encoding='latin-1',converters={'outline': eval,'candidats':eval})

        df_test=pd.read_csv(args.test_file,encoding='latin-1',converters={'outline': eval,'candidats':eval},nrows=args.test_subset)
        df_test=df_test.loc[df_test['outline'].map(len)>0 ]
        
        
        test_dataset= CustomDataset_planning.construct_from_raw(df_test,args.only_outlines,args.language_model,args.end2end)
        train_dataset= CustomDataset_planning.construct_from_raw(train_dataset,args.only_outlines,args.language_model,args.end2end)
        val_dataset= CustomDataset_planning.construct_from_raw(val_dataset,args.only_outlines,args.language_model,args.end2end)
    else:
        ...
        train_dataset=  CustomDataset_planning.construct_from_raw(df_train)
        val_dataset=  CustomDataset_planning.construct_from_raw(df_test)
    ...
    training_set = CustomDataset_planning(train_dataset, tokenizer, config.MAX_LEN, config.SUMMARY_LEN,config.OUTLINE_LEN)
    val_set = CustomDataset_planning(val_dataset, tokenizer, config.MAX_LEN, config.SUMMARY_LEN,config.OUTLINE_LEN)
    test_set = CustomDataset_planning(test_dataset, tokenizer, config.MAX_LEN, config.SUMMARY_LEN,config.OUTLINE_LEN)
    ...
    model=\
    T5Planning(train_val_test=(training_set,val_set,test_set), 
                ...
                max_input=args.max_input_size, max_output=args.max_output_size, max_outline_len=config.OUTLINE_LEN,
    ...
    checkpoint_callback = ModelCheckpoint(monitor="Val/loss_epoch", mode="min",save_top_k=2, every_n_val_epochs=2)

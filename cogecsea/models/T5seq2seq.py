...
#sys.path.append('/home/djeddal/Documents/Stage21/LifeLong-Learning-For-RI/Code')
...
from cogecsea.modules.planning_model import Planning_Seq2Seq, Planning_Seq2Seq_Teacher_forcing,Planning_Sequential
...
class T5Planning(planningSeqToSeq):
    ...
    def __init__(self, max_input=512, max_output=512, max_outline_len=128,
        ...
        try:
            eval_cache_dir = os.path.expandvars(cache_dir)
            text_model = T5ForConditionalGeneration.from_pretrained(os.path.join(eval_cache_dir, model_name+'-text-model'))
            if (init_model is not None):
                outline_model = T5ForConditionalGeneration.from_pretrained(init_model)
            else:
                outline_model = T5ForConditionalGeneration.from_pretrained(os.path.join(eval_cache_dir, model_name+'-outline-model'))
            if ptokenizer is None:
                self.tokenizer = T5Tokenizer.from_pretrained(os.path.join(eval_cache_dir, model_name+'-tokenizer'))
            else:
                self.tokenizer=ptokenizer
    ...
    def training_step(self, data, batch_idx):
        ...
        output=self.model(input_ids = {'source_ids':ids,'outline_ids':outline_y}, attention_mask = {'source_mask':mask,'outline_mask':outline_mask}, 
    ...
    def validation_step(self, data, batch_idx):
        ...
        with torch.no_grad():
            ...
            output=self.model(input_ids = {'source_ids':ids,'outline_ids':outline_y}, attention_mask = {'source_mask':mask,'outline_mask':outline_mask}, decoder_input_ids={'outline_ids':outline_y_ids, 
                'ful_answer_ids':y_ids}, labels={'outline_labels':outline_lm_labels, 'full_answer_labels':lm_labels }, validation=True)
            loss=output['sum_loss']
    ...
    def test_step(self, data, batch_idx):
        ...
        with torch.no_grad():
            ...
            gen=self.model.generate(input_ids = ids,
                ...
                length_penalty=1.0, 
                early_stopping=True)
            preds = [self.tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True) for g in gen['full_answer_ids']]
            outl = [self.tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True) for g in gen['outline_ids']]
...
class CustomDataset(Dataset):
    ...
    def construct_from_raw(cls,df,only_outline=False, language_model=False, end2end=False):
        ...
        for i_, row in filtered.iterrows():
            if (language_model):
                ...
                inputs.append("[Query:]"+row['query']+"[Documents:]"+"".join(["[Document:]"+c for c in row['candidats']]))
            if (only_outline):
    ...
    def __getitem__(self, index):
        ...
        source = self.tokenizer.batch_encode_plus([ctext],truncation=True, max_length= self.source_len, padding='max_length',return_tensors='pt')
        target = self.tokenizer.batch_encode_plus([text], truncation=True,max_length= self.summ_len, padding='max_length',return_tensors='pt')
...
class CustomDataset_planning(Dataset):
    ...
    def __getitem__(self, index):
        ...
        source = self.tokenizer.batch_encode_plus([ctext],truncation=True, max_length= self.source_len, padding='max_length',return_tensors='pt')
        target = self.tokenizer.batch_encode_plus([text], truncation=True,max_length= self.summ_len, padding='max_length',return_tensors='pt')
        outline = self.tokenizer.batch_encode_plus([outline], truncation=True,max_length= self.outline_len,  padding='max_length',return_tensors='pt')
